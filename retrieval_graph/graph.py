...
from backend_lcro.retrieval_graph.configuration import AgentConfiguration
from backend_lcro.retrieval_graph.researcher_graph.graph import graph as researcher_graph
from backend_lcro.retrieval_graph.state import AgentState, InputState, Router
from backend_lcro.utils import format_docs, load_chat_model
from langchain_core.runnables import ensure_config
# Local module imports
from backend_lcro.kvstore import SQLStore
from backend_lcro.constants import (
...
async def conduct_research(state: AgentState, *, config: RunnableConfig) -> dict[str, Any]:
    ...
    # Assuming `configuration` is an instance of AgentConfiguration
    configuration_dict = asdict(configuration)

    # Now pass the dictionary to `ensure_config`
    config = ensure_config(configuration_dict)
...
def get_unique_s3uri__documents(documents):
    """Filter out duplicate documents based on their `source` metadata."""
    cs = SQLStore(PARENT_DOC_DB_PATH, "docstore")
    docstore = create_kv_docstore(cs)
    messages = []
    for doc in documents:  
        if doc.metadata['doc_type'] == 'pdf_image' or doc.metadata['doc_type'] == 'image':
            retrieved_docs = docstore.mget([doc.metadata['doc_id']])
            if retrieved_docs and retrieved_docs[0]:
                retrieved_doc = retrieved_docs[0]
                image_message = {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/jpeg;base64,{retrieved_doc.page_content}"
                    },
                }
...
async def respond(
    ...
    text_message = {
        "type": "text",
        "text": (
             configuration.response_system_prompt.format(context=context) + "\nQuestion: " + state.messages[0].content
        ),
    }
   
    sources3_documents = get_unique_s3uri__documents(state.documents[:top_k])
    combined_messages = sources3_documents + [text_message]
    messages = [HumanMessage(content = combined_messages)]
    # print(messages)
    # sources3_documents.append(text_message)
    # prompt = configuration.response_system_prompt.format(context=context)
    # messages = [{"role": "system", "content": text_message}] + sources3_documents
    response = await model.ainvoke(messages)
    return {"messages": [response], "answer": response.content, "sources3":sources3_documents}
